...
function log(s) {
  ...
  if (typeof efi !== 'undefined' && efi.SystemTable && efi.SystemTable.ConOut) {
    try {
      efi.SystemTable.ConOut.OutputString(s + '\r\n');
      return;
    } catch (e) {}
  }
...
test('Promise.all exists', function () {
  return typeof Promise.all === 'function';
});
test('Promise.race exists', function () {
...
testSyntax('arrow fn', 'if ((() => 42)() !== 42) throw new Error("bad");');
testSyntax(
  'let / const',
...
testSyntax('exponent operator', 'if ((2 ** 3) !== 8) throw new Error("bad");');
testSyntax(
  'logical assignment (??=, ||=, &&=)',
